...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:@localhost/productos/productos/producto"
...
class producto(db.Model):  # Producto hereda de db.Model
    ...
    Calorias = db.Column(db.String(30))

    def __init__(self, nombre, Foto, Color, Calorias):
        """
        Constructor de la clase Producto.

        Args:
            nombre (str): Nombre del producto.
            Foto (str): URL o ruta de la imagen del producto.
            Color (int): Color del valor calorico del producto.
            Calorias (str): Nivel calorico del producto producto.
        """
        self.nombre = nombre
        self.Foto = Foto
        self.Color = Color
...
class ProductoSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre", "Foto", "Color", "Calorias")
...
'''
@app.route("/productos/productos/producto", methods=["GET"])
def get_producto():
    """
...
'''
@app.route("/productos/productos/producto/<id>", methods=["GET"])
def get_producto(id):
    """
...
@app.route("/productos/productos/producto/<id>", methods=["DELETE"])
...
@app.route("/productos/productos/producto", methods=["POST"])  # Endpoint para crear un producto
def create_producto():
    """
    ...
    Color = request.json["Color"]  # Obtiene el stock del producto del JSON proporcionado
    Calorias = request.json["Calorias"]  # Obtiene la imagen del producto del JSON proporcionado
    new_producto = producto(Nombre, Foto, Color, Calorias)  # Crea un nuevo objeto Producto con los datos proporcionados
...
@app.route("/productos/productos/producto/<id>", methods=["PUT"])  # Endpoint para actualizar un producto
def update_producto(id):
    ...
    producto.Foto = request.json["Foto"]
    producto.Color = request.json["Color"]
    producto.Calorias = request.json["Calorias"]
...
if __name__ == "__main__":
    # Ejecuta el servidor Flask en el puerto 3306 en modo de depuración
